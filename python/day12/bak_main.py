...
parts_checked = {}
def recurse(springs, nums, level, prev, initial, index):
    ...
    for i in range(len(springs)):
        ...
        if not valid:
            if done:
                break
            continue
        total += recurse(springs[i+num_hashes+1:], nums, level+1, prev + "."*i + "#"*num_hashes + ".", initial, index)
        if done:
            break
    return total
...
def part1(inp):
    ...
    for i in range(len(inp)):
        spring = inp[i]
        #break
        tmp = recurse(spring[0], spring[1], 1, "", spring[0], i)
        #print(tmp)
    ...
    if False:
        ...
        print(inp[test][0])
        print("-----------------")
        brute_force(inp[test][0], inp[test][1])
        print("--------------------")
        check = [{}]
        recurse(inp[test][0], inp[test][1], 1, "", spring[0], i)
        #print(recurse(inp[test][0], inp[test][1], 1, "", inp[test][0]))
